...
import { FaHome, FaUsers, FaCalendarAlt, FaChartLine, FaSignOutAlt, FaBars, FaTimes, FaUserMd, FaRobot } from 'react-icons/fa';
...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <button className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-white rounded-lg shadow-md" onClick={() => setSidebarOpen(!sidebarOpen)}>
      ...
      <aside className={'fixed top-0 left-0 z-40 w-64 h-screen bg-gradient-to-b from-blue-800 to-blue-900 transition-transform duration-300 ease-in-out ' + (sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
        <div className="p-5">
          ...
          <nav className="space-y-1">
            {menuItems.map((item) => (
              <button key={item.path} onClick={() => { navigate(item.path); setSidebarOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-gray-200 rounded-lg hover:bg-white/10 transition group">
                <item.icon size={18} />
        ...
        <div className="absolute bottom-0 w-full p-5 border-t border-white/10">
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-gray-200 rounded-lg hover:bg-red-600/20 transition">
